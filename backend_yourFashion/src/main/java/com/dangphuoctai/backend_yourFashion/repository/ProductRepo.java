...
public interface ProductRepo extends JpaRepository<Product, Long> {
    ...
    @Query(value = "SELECT * FROM products p WHERE p.discount != 0 AND p.product_name REGEXP :keyword", countQuery = "SELECT COUNT(*) FROM products p WHERE p.discount != 0 AND p.product_name REGEXP :keyword", nativeQuery = true)
    Page<Product> findByProductNameRegexp(String keyword, Pageable pageable);

    @Query(value = "SELECT * FROM products p WHERE p.discount != 0 AND p.product_name REGEXP :keyword", countQuery = "SELECT COUNT(*) FROM products p WHERE p.discount != 0 AND p.product_name REGEXP :keyword", nativeQuery = true)
    Page<Product> findByNotDiscountAndProductNameRegexp(@Param("keyword") String keyword, Pageable pageable);
